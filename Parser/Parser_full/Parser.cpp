 ...
 *
 /--------------------------------------------------------*/
void Parser::Get_func_call(Node *function)
{
    if ((function->knot_.storage_.oper_ != CALL)
        || (function->right_node_ != nullptr)
        || function->left_node_ != nullptr)
    {
            THROW(LOGIC_ERROR, "Incorrect input!", nullptr);
    }
    ...
    function->left_node_ = new Node(param_amount);
}
...
char* Parser::Get_func_name()
{
    ...
    func_list_->add_variable(INTEGER, func_list_->name_buffer_);
